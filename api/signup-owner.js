...
export default async function handler(req, res) {
  ...
  if (!process.env.ADMIN_SIGNUP_CODE) {
    return res
      ...
      .json({
        error:
          "সার্ভারে ADMIN_SIGNUP_CODE সেট করা নেই — Vercel Environment Variables চেক করুন",
      });
  }
  ...
  if (!shop_name || !full_name || !email || !password) {
    return res
      .status(400)
      .json({
        error: "দোকানের নাম, আপনার নাম, ইমেইল, পাসওয়ার্ড — সবগুলো লাগবে",
      });
  }
  ...
  try {
    ...
    if (msg.includes("already") || msg.includes("registered")) {
      return res
        .status(400)
        .json({ error: "এই ইমেইল দিয়ে আগে থেকেই একটি অ্যাকাউন্ট আছে" });
    }
    return res
      .status(500)
