...
function collectState(isNewShop){
 ...
 suppliers, supplierNextId, supplierDueEntries, supplierDueNextId,
...
function switchView(id, opts){
 ...
 if(id === 'ledger'){ ledgerSearch = ''; ledgerDetailId = null; }
 if(id === 'employees'){ employeeDetailId = null; }
 if(id === 'suppliers'){ supplierDetailId = null; }
 ...
 const titles = {dashboard:'ড্যাশবোর্ড', sales:'বিক্রয়', cart:'কার্ট', checkout:'ইনভয়েস তৈরি করুন', invoicePreview:'ইনভয়েস', paymentReceipt:'প্রাপ্তি রশিদ', stock:'স্টক তালিকা', purchaseLedger:'কেনার খাতা', salesLedger:'বেচার খাতা', ledger:'বাকির খাতা', dueSummary:'বাকির হিসাব (দিয়েছি/পেয়েছি)', employees:'কর্মচারী', suppliers:'সাপ্লায়ার', cash:'নগদ ক্রেতা', expenses:'খরচ', invoices:'ইনভয়েস হিস্ট্রি', returns:'রিটার্ন/এক্সচেঞ্জ', daily:'দৈনিক হিসাব', profit:'লাভ-ক্ষতি', report:'ব্যবসার রিপোর্ট', aiAssistant:'AI সহকারী', cashbox:'ক্যাশবক্স', staff:'স্টাফ ও লগ', settings:'দোকানের তথ্য'};
 ...
 } catch(e){ /* history API না থাকলে সাইলেন্টলি বাদ */ }
 }
}
...
function renderCartPage(){
 ...
 <div class="cart-total-row"><span>মোট পিস</span><span class="mono">${totalPieces}</span></div>
...
function editBrandPrompt(name){
 openModal(`ব্র্যান্ড এডিট — ${esc(name)}`, `
...
function saveBrandRename(oldName){
 ...
 if(!newName){ showToast('ব্র্যান্ডের নাম আবশ্যক'); return; }
 if(newName !== oldName && BRANDS.includes(newName)){ showToast('এই নামে আরেকটা ব্র্যান্ড আগে থেকেই আছে'); return; }
 if(newName !== oldName){
 ...
 closeModal(); render(); showToast('ব্র্যান্ড আপডেট হয়েছে'); persistShopData();
}
function deleteBrandPrompt(name){
 const itemCount = Object.keys(inventory[name]||{}).length;
 openModal('ব্র্যান্ড মুছবেন?', `
...
function deleteBrandConfirmed(name){
 ...
 closeModal(); render(); showToast('ব্র্যান্ড মুছে ফেলা হয়েছে'); persistShopData();
}
...
function addStockRecalc(){
 ...
 if(sellEl && !stockSellManualOverride) sellEl.value = buyPerPiece > 0 ? (buyPerPiece + 10) : '';
}
function editStockRecalc(sz){
 ...
 if(sellEl && !stockSellManualOverride) sellEl.value = buyPerPiece > 0 ? (buyPerPiece + 10) : sellEl.value;
}
...
function addStockPrompt(){
 ...
 <div class="field"><label>সাইজ / ফুট (তালিকা থেকে বাছুন বা নিজে লিখুন)</label>
...
function editStockPrompt(brand, mm, sz){
 ...
 openModal(`এডিট — ${esc(brand)} · ${mm}মি:লি: · ${sz}ফুট`, `
...
function saveStockEdit(brand, mm, sz){
 ...
 if(!newMM || !newSize){ showToast('মি:লি: ও ফুট আবশ্যক'); return; }
 ...
 const collision = inventory[brand][newMM] && inventory[brand][newMM][newSize];
 if(collision){
 showToast('এই মি:লি: ও ফুটের আইটেম আগে থেকেই আছে — অন্য মান দিন');
 return;
 }
 delete inventory[brand][mm][sz];
 if(Object.keys(inventory[brand][mm]).length === 0) delete inventory[brand][mm];
 if(!inventory[brand][newMM]) inventory[brand][newMM] = {};
 inventory[brand][newMM][newSize] = v;
 logActivity('স্টক আইটেমের মি:লি:/ফুট সংশোধন', `${brand} · ${mm}মি:লি:→${newMM}মি:লি:, ${sz}ফুট→${newSize}ফুট`);
 }
...
function recordPurchase(brand, mm, sz, banQty, banPrice, pieces, buyPerPiece){
 ...
 logActivity('নতুন মাল ক্রয়/যোগ', `${brand} · ${mm}মি:লি: · ${sz}ফুট · ${pieces} পিস · খরচ ${fmt(cost)}`);
}
...
function openSupplierDetail(id){ supplierDetailId = id; render(); }

function addSupplierPrompt(){
 openModal('নতুন সাপ্লায়ার যুক্ত করুন', `
...
function saveNewSupplier(){
 ...
 if(!name){ showToast('নাম আবশ্যক'); return; }
 const startDue = Math.max(0, parseInt(document.getElementById('supStartDue').value) || 0);
 const sup = {
 id: supplierNextId++, name,
 phone: document.getElementById('supPhone').value.trim(),
 address: document.getElementById('supAddress').value.trim(),
 brands: document.getElementById('supBrands').value.trim(),
 due: startDue, paidTotal: 0,
 };
 suppliers.push(sup);
 logActivity('নতুন সাপ্লায়ার যুক্ত', `${name}${startDue>0?' · শুরুর বাকি '+fmt(startDue):''}`);
 closeModal(); render(); showToast('সাপ্লায়ার যুক্ত হয়েছে'); persistShopData();
}
function editSupplierPrompt(id){
 const s = suppliers.find(x=>x.id===id);
 if(!s) return;
 openModal(`সাপ্লায়ারের তথ্য — ${esc(s.name)}`, `
...
function deleteSupplierConfirmed(id){
 ...
 closeModal(); render(); showToast('সাপ্লায়ার মুছে ফেলা হয়েছে');
...
function addSupplierDuePrompt(id){
 const s = suppliers.find(x=>x.id===id);
 if(!s) return;
 openModal(`ক্রয়/বাকি যোগ করুন — ${esc(s.name)}`, `
...
function saveSupplierDue(id){
 const s = suppliers.find(x=>x.id===id);
 if(!s) return;
 const amount = Math.max(0, parseInt(document.getElementById('supDueAmount').value) || 0);
 if(amount <= 0){ showToast('সঠিক পরিমাণ লিখুন'); return; }
 ...
 supplierDueEntries.push({ id: supplierDueNextId++, supplierId: id, date, amount, note });
 s.due = (s.due || 0) + amount;
 logActivity('সাপ্লায়ারের কাছে নতুন বাকি যোগ', `${s.name} · ${fmt(amount)}${note?' · '+note:''}`);
 closeModal(); render(); showToast('বাকি যোগ হয়েছে'); persistShopData();
}

function supplierPaymentPrompt(id){
 const s = suppliers.find(x=>x.id===id);
 if(!s) return;
 openModal(`পরিশোধ করুন — ${esc(s.name)}`, `
...
function saveSupplierPayment(id){
 const s = suppliers.find(x=>x.id===id);
 if(!s) return;
 const amount = Math.max(0, parseInt(document.getElementById('supPayAmount').value) || 0);
 if(amount <= 0){ showToast('সঠিক পরিমাণ লিখুন'); return; }
 ...
 const cat = expenseCategories.find(c=>c.id===catId);
 expenses.push({
 id: expenseNextId++, date,
 categoryId: catId, categoryName: cat ? cat.name : 'সাপ্লায়ার পরিশোধ',
 personId: null, personName: s.name, supplierId: id, amount,
 note: note || `মাধ্যমঃ ${method}`,
 });
 logActivity('সাপ্লায়ারকে পরিশোধ', `${s.name} · ${fmt(amount)} (${method})`);
 closeModal(); render(); showToast(`${fmt(amount)} পরিশোধ করা হয়েছে ও খরচে যোগ হয়েছে`); persistShopData();
}
...
function renderSuppliers(){
 ...
 return toolbar + `<div class="empty-state"><div class="ic">🚚</div>এখনো কোনো সাপ্লায়ার যুক্ত করা হয়নি<br><span style="font-size:12px;">"+ নতুন সাপ্লায়ার" চেপে শুরু করুন</span></div>`;
 }
...
function renderSupplierDetail(id){
 const s = suppliers.find(x=>x.id===id);
 if(!s){ supplierDetailId = null; return renderSuppliers(); }

 const totalOwed = supplierDueEntries.filter(d=>d.supplierId===id).reduce((sum,d)=>sum+d.amount,0);
 const totalPaid = expenses.filter(e=>e.supplierId===id).reduce((sum,e)=>sum+e.amount,0);
 ...
 const combined = [
 ...supplierDueEntries.filter(d=>d.supplierId===id && inSelectedPeriod(d.date, period)).map(d => ({ t: new Date(d.date).getTime(), html: `
 <div class="day-tx">
 <div>
 ...
 ...expenses.filter(e=>e.supplierId===id && inSelectedPeriod(e.date, period)).map(e => ({ t: new Date(e.date).getTime(), html: `
 <div class="day-tx">
 <div>
 ...
 const txHtml = combined.length === 0 ? `<div class="no-match">এই সময়ে কোনো লেনদেন নেই</div>` : combined.map(x=>x.html).join('');

 return backRow + headerPanel + statsPanel + periodTabsHtml('suppliers') + `
 <div class="panel">
...
function openCustomerDetail(id){ ledgerDetailId = id; render(); }

function renderCustomerDetail(id){
 const cust = ledger.find(l => l.id === id);
 if(!cust){ ledgerDetailId = null; return renderLedger(); }

 const custInvoices = invoices.filter(inv => inv.custId === id && !inv.cancelled);
 ...
 const combined = [
 ...custInvoices.filter(inv => inSelectedPeriod(inv.date, period)).map(inv => ({ t: new Date(inv.date).getTime(), html: `
 <div class="day-tx">
 <div>
 ...
 ...custPayments.filter(p => inSelectedPeriod(p.date, period)).map(p => ({ t: new Date(p.date).getTime(), html: `
 <div class="day-tx">
 <div>
 ...
 return backRow + headerPanel + statsPanel + periodTabsHtml('customerDetail') + `
 <div class="panel">
 <h3>লেনদেনের ইতিহাস</h3>
 ${txHtml}
 </div>`;
...
function renderCashbox(){
 ...
 if(inv.paid > 0) txns.push({ t: new Date(inv.date).getTime(), date: inv.date, dir:'in', cat:'sale', label:`বিক্রয় — ${inv.customer}`, detail:`ইনভয়েস #${inv.id}`, amount: inv.paid, invoiceId: inv.id });
 });
 payments.forEach(p => {
 ...
 txns.push({ t: new Date(pu.date).getTime(), date: pu.date, dir:'out', cat:'purchase', label:`কেনা — ${pu.brand}`, detail:`${pu.mm}মি:লি: · ${pu.size}ফুট · ${pu.banQty} বান`, amount: pu.cost });
 });
 expenses.forEach(e => {
...
function openCashboxMemoDetail(invId){
 ...
 const returnedRows = relatedReturns.map(r => `
 ...
 const cancelledBanner = inv.cancelled ? `<div style="background:#FCEBE9;color:var(--red);border-radius:8px;padding:10px 14px;font-size:12.5px;text-align:center;font-weight:700;margin-bottom:12px;">❌ এই ইনভয়েসটি বাতিল করা হয়েছে</div>` : '';
...
function renderBusinessReport(){
 ...
 <tr><td>${esc(b)}</td><td class="num r mono">${v.qty}</td><td class="num r mono">${fmt(v.revenue)}</td></tr>`).join('');
