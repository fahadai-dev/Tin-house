...
function renderDashboard() {
  ...
  expenses.forEach((e) => {
    if (dayKey(e.date) === todayKey) todayExpense += e.amount;
  });
 ...
 <button class="btn btn-outline" style="padding:4px 9px;font-size:11.5px;" onclick="callCustomer(${l.id})">📞 কল</button>
     ...
     .join("") ||
   `<div class="empty-state" style="padding:20px;"><div class="ic">✅</div>কোনো বাকি নেই</div>`
 }
 </div>`;
}
...
function renderPurchaseLedger() {
 ...
 <span class="sclear" onclick="purchaseSearchInputFn('')">✕</span>
  ...
  if (filtered.length === 0)
    return (
      searchBar +
      `<div class="no-match">🔍 "${esc(purchaseSearch)}" এর সাথে মিলে এমন কোনো ক্রয় পাওয়া যায়নি</div>`
    );
...
function renderSalesLedger() {
 ...
 <span class="sclear" onclick="salesLedgerSearchInputFn('')">✕</span>
  ...
  if (filtered.length === 0)
    return (
      searchBar + `<div class="no-match">🔍 কোনো ফলাফল পাওয়া যায়নি</div>`
    );
...
function renderInvoices() {
 ...
 <span class="sclear" onclick="invoiceSearchInputFn('')">✕</span>
  ...
  if (filtered.length === 0)
    return (
      searchBar +
      `<div class="no-match">🔍 "${invoiceSearch}" এর সাথে মিলে এমন কোনো ইনভয়েস পাওয়া যায়নি</div>`
    );
...
function renderBusinessReport() {
  ...
  const filteredInv = invoices.filter((inv) =>
    reportInRange(inv.date, range.from, range.to),
  );
