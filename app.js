...
function switchView(id){
 ...
 const titles = {dashboard:'ড্যাশবোর্ড', sales:'বিক্রয়', cart:'কার্ট', checkout:'ইনভয়েস তৈরি করুন', invoicePreview:'ইনভয়েস', stock:'স্টক তালিকা', purchaseLedger:'কেনার খাতা', salesLedger:'বেচার খাতা', ledger:'বাকির খাতা', cash:'নগদ ক্রেতা', expenses:'খরচ', invoices:'ইনভয়েস হিস্ট্রি', returns:'রিটার্ন/এক্সচেঞ্জ', daily:'দৈনিক হিসাব', profit:'লাভ-ক্ষতি', report:'ব্যবসার রিপোর্ট', aiAssistant:'AI সহকারী', staff:'স্টাফ ও লগ', settings:'দোকানের তথ্য'};
...
function renderDashboard(){
 let totalStock = 0, totalDue = 0, todaySales = 0, todayCollected = 0, todayReceivedFromSales = 0, todayExpense = 0;
 ...
 return `
 <div class="dash-hero">
 <div class="dh-label">আজকের বিক্রয়</div>
...
function renderPurchaseLedger(){
 ...
 if(purchases.length === 0) return `<div style="font-size:13px;color:var(--steel-500);margin-bottom:14px;">দোকানের সব ক্রয়ের হিসাব এখানে স্বয়ংক্রিয়ভাবে যুক্ত হয় — "স্টক তালিকা" থেকে নতুন মাল বা নতুন বান যোগ করুন</div>` +
 ...
 if(filtered.length === 0) return searchBar + `<div class="no-match">🔍 "${esc(purchaseSearch)}" এর সাথে মিলে এমন কোনো ক্রয় পাওয়া যায়নি</div>`;
...
function renderSalesLedger(){
 ...
 if(filtered.length === 0) return searchBar + `<div class="no-match">🔍 কোনো ফলাফল পাওয়া যায়নি</div>`;
...
function renderExpenseEntriesTab(){
 ...
 body = `<div class="no-match">🔍 এই সার্চের সাথে মিলে এমন কোনো খরচ পাওয়া যায়নি</div>`;
 } else {
 ...
 return catGrid + filterBanner + searchBar + body;
}
...
function renderReturns(){
 ...
 <tbody>${returns.slice().reverse().map(r => `
 <tr>
...
function renderStaff(){
 ...
 <h3 style="margin-bottom:0;">স্টাফ তালিকা</h3>
